                ...
                if (character === '[' && !numeric) {
                    ...
                    try {
                        ...
                        if (equation.charAt(i + 1) === '(' || (equation.charAt(i + 1) === '[' && equation.charAt(i + 2) === '(')) {
                            ...
                            var args = equation.substring(i, endSpecial).replace(/[\[\]]/g, '');
                            args = args.split(',');
                            var originalArgs = args.slice(0);
                            for (var j = 0; j < args.length; j++) {
                                args[j] = calculate(args[j], specOp !== 'date');
                                if (args[j] && specOp !== 'date') {
                            ...
                            if (specOp === 'dateString') {
                                ...
                                if (args.length === 1 && typeof args[0] === 'string') {
                                    dateArr = args[0].split(/[\-\/\.]/g);
                                }
                                if (args.length > 2 || dateArr.length > 2) {
                                    var computeArr = dateArr.length > 2 ? dateArr : args;
                                    var millis = Date.UTC(computeArr[2], computeArr[1] - 1, computeArr[0]);
